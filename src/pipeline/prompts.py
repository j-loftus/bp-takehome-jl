"""
Extraction prompt constants and builder — Task 1.3.

Implements the single-template-with-injected-blocks strategy from
docs/prompt_template.md. Consumed by extractor.py (Task 2.3).

Usage:
    from src.pipeline.prompts import build_extraction_prompt
    prompt = build_extraction_prompt(doc_type="renewal_letter", document_text=text)
"""
...
#
# Three placeholders:
#   {doc_type}        — the classified document type (enum value string)
#   {injected_fields} — doc-type-specific field definitions (from INJECTED_FIELDS)
#   {document_text}   — the raw text extracted from the PDF
#
# Literal JSON braces in the example output section are doubled ({{ }}) to
# survive Python str.format() substitution.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Doc-type-specific field blocks
#
# Each value is a string of field definitions injected into the template at
# runtime via {injected_fields}. Keys are DocType enum value strings.
# ---------------------------------------------------------------------------
...
- Extraction rule: If an explicit end date is stated in the document, extract it directly.
  If no explicit end date is stated, but the document states both (a) a start/execution date
  and (b) a fixed duration (e.g., "195 calendar days from execution", "a term of one (1) year"),
  CALCULATE the end date by adding the duration to the start date and return the computed value.
  Do not leave this field null solely because the end date is not stated verbatim — derive it
  whenever the inputs needed to do so are present in the document. If neither an explicit date
  nor a start date + duration pair is available, return null.
- Provenance flag: If this value was calculated rather than directly stated in the document,
  note it in extraction_notes, e.g.: "contract_end_date calculated from start date 2024-11-04
  plus 195-day duration; not explicitly stated in document." This preserves the distinction
  between a literal extraction and a derived value for downstream review.
...
def build_extraction_prompt(doc_type: str, document_text: str) -> str:
    """
    Build the extraction prompt for a given document type and text.

    Looks up the doc-type-specific field block from INJECTED_FIELDS and
    substitutes it into EXTRACTION_PROMPT_TEMPLATE along with the document text.
    Falls back to the "other" field block for any unrecognized doc_type value.
    """
